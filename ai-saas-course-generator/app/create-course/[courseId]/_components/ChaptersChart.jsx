...
function ChaptersChart({course,refreshData,edit=true}) {
  return (
    <div className='mt-3'>
      ...
      <div className='mt-2'>
        {course?.courseOutput?.course.chapters.map((chapter,index)=>{
          <div className='border p-5 rouned-lg mb-2 flex items-center justify-between'>
          <div className='flex gap-5 items-center'>
            <h2 className='bg-primary  flex-none h-10 w-10 text-white rounded-full text-center p-2'>{index+1}</h2>
            <div>
              {edit&&<h2 className='font-medium text-lg'>{chapter?.name}<EditChapters course={course} index={index} refreshData={refreshData}/></h2>}
